...
def setup(filename, outfile, x_lim, y_lim, jk):
    ...
    if maps.ndim == 5:
        mapData[0,:,:] = maps[0,sb-1,freq-1,:,:]
        mapData[1,:,:] = maps[1,sb-1,freq-1,:,:]
        hitData[0,:,:] = hits[0,sb-1,freq-1,:,:]
        hitData[1,:,:] = hits[1,sb-1,freq-1,:,:]
        rmsData[0,:,:] = rms[0,sb-1,freq-1,:,:]
        rmsData[1,:,:] = rms[1,sb-1,freq-1,:,:]
        for p in range(nx):
            for q in range(ny):
        ...
        if len(det_list) == 1:
            det = det_list[0]
            mapData[0,:,:] = maps[0,det-1,sb-1,freq-1,:,:]
            mapData[1,:,:] = maps[1,det-1,sb-1,freq-1,:,:]
            hitData[0,:,:] = hits[0,det-1,sb-1,freq-1,:,:]
            hitData[1,:,:] = hits[1,det-1,sb-1,freq-1,:,:]
            rmsData[0,:,:] = rms[0,det-1,sb-1,freq-1,:,:]
            rmsData[1,:,:] = rms[1,det-1,sb-1,freq-1,:,:]
            for p in range(nx):
                for q in range(ny):
                    mapData[2,q,p] = mapData[0,q,p]/rmsData[0,q,p]**2 + mapData[1,q,p]/rmsData[1,q,p]**2
                    rmsData[2,q,p] = 1./rmsData[0,q,p]**2 + 1./rmsData[1,q,p]**2
            ...
            for p in range(nx):
                for q in range(ny):
                    for det in det_list:
                        mapData[0,q,p] += maps[0,det-1,sb-1,freq-1,q,p]/rms[0,det-1,sb-1,freq-1,q,p]**2
                        mapData[1,q,p] += maps[1,det-1,sb-1,freq-1,q,p]/rms[1,det-1,sb-1,freq-1,q,p]**2
                        hitData[0,q,p] += hits[0,det-1,sb-1,freq-1,q,p]
                        hitData[1,q,p] += hits[1,det-1,sb-1,freq-1,q,p]
                        rmsData[0,q,p] += 1./rms[0,det-1,sb-1,freq-1,q,p]**2
                        rmsData[1,q,0] += 1./rms[1,det-1,sb-1,freq-1,q,p]**2
                        mapData[2,q,p] += maps[0,det-1,sb-1,freq-1,q,p]/rms[0,det-1,sb-1,freq-1,q,p]**2 + maps[1,det-1,sb-1,freq-1,q,p]/rms[1,det-1,sb-1,freq-1,q,p]**2
                        rmsData[2,q,p] += 1./rms[0,det-1,sb-1,freq-1,q,p]**2 + 1./rms[1,det-1,sb-1,freq-1,q,p]**2
                        hitData[2,q,p] += hits[0,det-1,sb-1,freq-1,q,p] + hits[1,det-1,sb-1,freq-1,q,p]
...
def makeMap(x, y, plotData, hits, mapname, fig, ax0, ax1, ax2, ax3, x_lim, y_lim, jk):
    ...
    if plot == 'hit':
        im = ax0.imshow(plotData[0,:,:], extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax1.imshow(plotData[1,:,:], extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax3.imshow(plotData[3,:,:], extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax2.imshow(plotData[2,:,:], extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
    else:
        im = ax0.imshow(plotData[0,:,:]*1e6, extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax1.imshow(plotData[1,:,:]*1e6, extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax2.imshow(plotData[2,:,:]*1e6, extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
        im = ax3.imshow(plotData[3,:,:]*1e6, extent=(x_lim[0],x_lim[1],y_lim[0],y_lim[1]), interpolation='nearest', aspect='equal', cmap=cm.CMRmap, origin='lower', vmin=color_lim[0], vmax=color_lim[1])
